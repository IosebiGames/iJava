...
What I wanted to do wasn't sophisticated. I wanted to run Java code on iOS (which doesn't use any External libraries and isn't GUI-Based), to let it write basic "Experiment.txt" file and print anything in console (like "GUI-Less Java Code Running on iOS!"). Since I'm not an iOS developer and never was, I was aiming to let this Java code to run on any versions of iOS but then I remembered about my iPhone 5s which is iOS 12.5.8 (even though the version itself is old, but trying to push limits and achieve what I wanted still felt like a goal). since we already know that Java also has Runtime translation tools made by it's community (one of well-remembered ones are J2Objective-C) which I exactly tried to use at the beginning for this experiment.</br>
